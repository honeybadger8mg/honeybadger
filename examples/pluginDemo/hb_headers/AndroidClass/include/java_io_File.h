...
#ifndef HONEYBADGER_JAVA_IO_FILE_H
#define HONEYBADGER_JAVA_IO_FILE_H

#include "ISimulateClass.h"



class java_io_File : public ISimulateClass {
public:
    java_io_File(HB::EntryInterface *pDebugger);
...
public:
    void SetAbsFullPath(string strFullPath);
    string GetAbsFullPath();
...
private:
    ...
    string m_strAbsFullPath; // 文件的完整路径

    HB::EntryInterface *m_pEntry;
};


#endif //HONEYBADGER_JAVA_IO_FILE_H
