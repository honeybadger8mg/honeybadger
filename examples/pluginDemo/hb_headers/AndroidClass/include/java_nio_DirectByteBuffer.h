//
// Created by ice on 2022/7/16.
//

#ifndef HONEYBADGER_JAVA_NIO_DIRECTBYTEBUFFER_H
#define HONEYBADGER_JAVA_NIO_DIRECTBYTEBUFFER_H

#include "ISimulateClass.h"

class java_nio_DirectByteBuffer : public ISimulateClass {
public:
    java_nio_DirectByteBuffer(HB::EntryInterface *pDebugger);
...
public:
    // 获取缓冲区地址
    void SetDirectBufferAddress(uint32_t iBufAddr);
    uint32_t GetDirectBufferAddress();
...
private:
    ...
    uint32_t m_iVirtualBufAddr; // 要操作的虚拟内存地址
};


#endif //HONEYBADGER_JAVA_NIO_DIRECTBYTEBUFFER_H
