//
// Created by ice on 2022/7/17.
//

#ifndef HONEYBADGER_VA_JLONGARRAY_H
#define HONEYBADGER_VA_JLONGARRAY_H

#include <PreDefine.h>
#include "ISimulateClass.h"
#include <vector>

class VA_JLongArray : public ISimulateClass {
public:
    VA_JLongArray(HB::EntryInterface *pDebugger);
    ...
    uint64_t GetLongArrayElement(int iIndex);
    void AddNewValue(uint64_t lNewValue);
    void AddNewValue(uint64_t lNewValue, int iIndex);
...
public:
    static VA_JLongArray *CreateNew(HB::EntryInterface *pEntry, int iSize);
    void PreAllocEle(int iSize);
...
#endif //HONEYBADGER_VA_JLONGARRAY_H
