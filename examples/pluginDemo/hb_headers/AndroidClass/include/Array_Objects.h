//
// Created by ice on 2020/5/28.
//

#ifndef HONEYBADGER_ARRAY_OBJECTS_H
#define HONEYBADGER_ARRAY_OBJECTS_H
...
//  对应NewObjectArray的处理
class Array_Objects  : public ISimulateClass {
public:
    Array_Objects(HB::EntryInterface *pDebugger);
    ...
    ISimulateClass* GetElementValue(int iIndex);
...
public: // 数组 所以需要提前设置大小  如果没有设置大小的话，就直接在后面插入
    void SetArraySize(int iSize);
    void SetArrayElement(int iIndex, ISimulateClass *pAdd);
    void AddElement(ISimulateClass *pAdd);
...
private:
    ...
    vector<ISimulateClass*> m_vecElements; // 数组的所有成员

    HB::EntryInterface *m_pEntry;
};


#endif //HONEYBADGER_ARRAY_OBJECTS_H
