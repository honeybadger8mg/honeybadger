//
// Created by ice on 2020/7/17.
//

#ifndef HONEYBADGER_ANDROID_CONTENT_PM_PACKAGEMANAGER_H
#define HONEYBADGER_ANDROID_CONTENT_PM_PACKAGEMANAGER_H

#include "ISimulateClass.h"



class android_content_pm_PackageManager  : public ISimulateClass {
public:
    static android_content_pm_PackageManager *GetSingleInstance(HB::EntryInterface *pDebugger);

public:
    android_content_pm_PackageManager(HB::EntryInterface *pDebugger);
...
private:
    static unordered_map<string, android_content_pm_PackageManager *> m_mapInstaces; // key: 包名  每个包名就一个实例
};


#endif //HONEYBADGER_ANDROID_CONTENT_PM_PACKAGEMANAGER_H
